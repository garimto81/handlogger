...
## 🚀 v3.9.18 (2025-01-19) - 시간 매칭 디버깅 강화 (P0 Critical - Time Debugging)

### Debugging Enhancements
- 🔍 **서버/클라이언트 시간 디버깅 로그 추가**: `started_at_local` 전체 흐름 추적
  - **목적**: 왜 17:54로 매칭되는지 근본 원인 파악
  - **추가 로그**:
    - 📌 클라이언트 (핸드 등록): 브라우저 타임존, started_at, started_at_local
    - 📌 서버 (VIRTUAL 매칭): head.started_at_local 값, fallback 사용 여부, 최종 hhmmTime
  - **확인 항목**:
    1. 클라이언트가 올바른 started_at_local 전송하는지
    2. 서버가 started_at_local을 정상 저장/읽기하는지
    3. Fallback 함수 호출 여부
  - **파일**:
    - [index.html:1121-1126](index.html#L1121) - 클라이언트 로그
    - [code.gs:1082-1090](code.gs#L1082) - 서버 로그

### Next Steps
1. 새 핸드 등록 시 브라우저 콘솔 확인
2. VIRTUAL 전송 시 Apps Script 로그 확인
3. 로그 기반 근본 원인 파악
