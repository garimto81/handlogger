...
**문서 버전**: 2.0.0 | **최종 업데이트**: 2025-10-14 | **5분 완독용**
...
| 평균 핸드 기록 시간 | < 3분 | 2.8분 |
| **테이블 선택 시간 (v2.9.0)** | **< 0.5초** | **0.1초 (93% 절감)** |
| 지원 테이블 수 | 36개 | 36개 ✅ |

---

## 🚀 v2.8.0 & v2.9.0 주요 변경사항
...
|------|------|------|
| getConfig() | < 2초 | 1.2초 ✅ |
| **테이블 선택 (v2.9.0)** | **< 0.5초** | **0.1초 ✅** |
| saveHand() | < 3초 | 2.5초 ✅ |
| VIRTUAL 전송 | < 5초 | 3.2초 ✅ |

**최적화 기법:**
- SessionStorage 캐싱 (1분 TTL)
- RequestAnimationFrame 렌더링
- Bulk Insert (ACTIONS 시트)
- **v2.9.0**: 클라이언트 정렬 (서버 부하 감소)
...
### 단기 (2025 Q4)
- **v3.0.0 (2025-11)**: 사이드팟 자동 계산
- **v3.1.0 (2025-12)**: 멀티 테이블 병렬 지원 (탭 UI)
...
5. **UX 개선**: 키플레이어 우선 정렬 (v2.9.0)

### 차별화 요소
- Google Sheets 기반 **제로 인프라**
- 기여액 기반 **정확한 팟 계산**
- 키플레이어 **자동 자막 생성**
- **v2.9.0**: 테이블 선택 시간 93% 절감
